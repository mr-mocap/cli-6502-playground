...
static int WritePrimitiveRecord(SRecord::Buffer &buffer, const SRecord &record, const int bytes_in_address)
{
    ...
    snprintf( buffer.data(), buffer.size(), "S%c%0.2X%0.4X",
              record.type + '0',
    ...
    for (size_t byte_number_in_data = 0; byte_number_in_data < record.data.size(); ++byte_number_in_data)
    {
        ...
        snprintf( &buffer[ buffer_index_from_byte_number ], 3, "%0.2X", record.data[byte_number_in_data] );
    }
    ...
    snprintf( &buffer[length_up_to_checksum], 3, "%0.2X", checksum);
